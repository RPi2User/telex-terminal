...
import math
import struct
#import scipy.signa#l.signaltools as sigtool
...
class TelexED1000SC(txBase.TelexBase):
    def __init__(self):
        ...
        self.recv_squelch = 100
        self.recv_debug = False
        self.send_WB_pulse = False
        self.unres_threshold = 100
    ...
    def thread_rx(self):
        ...
        while self._run:
            ...
            # ms ("slow scan").
            #
            # When the operator presses AT, the teleprinter sends Z leve#l.
            ...
            #
            # The responsiveness delay is about 2x scan interva#l. (The receive
            ...
            if bit_last != bit and not (ST.ONLINE <= self._rx_state < ST.OFFLINE_DELAY):
                if bit is None:
                    print("[rx] Squelch active, last bit {}, previous counter value: {}".format(
                        "Z" if bit_last else "A",
                        _bit_counter_1 if bit_last else _bit_counter_0
                        )
                    )
                elif bit_last is None:
                    print("[rx] Squelch off, bit changed to {}, previous counter value: {}".format(
                        "Z" if bit else "A",
                        _bit_counter_0
                        )
                    )
                else:
                    print("[rx] Bit changed to {}, previous counter value: {}".format(
                        "Z" if bit else "A",
                        _bit_counter_1 if bit_last else _bit_counter_0
                        )
                    )
            ...
            if self._rx_state <= ST.OFFLINE: # ====================
                ...
                # Wait 3000 ms until switching to A leve#l.
                if offline_delay_counter > 600:
                    self._rx_state = ST.OFFLINE_WAIT
                    offline_delay_counter = 0
                else:
                    if offline_delay_counter % 100 == 0:
                        print("[rx] Offline delay running: {!r}/600".format(offline_delay_counter))
            ...
            if state_before != self._rx_state:
                print("[rx] State transition: {!s}=>{!s}".format(state_before, self._rx_state))
